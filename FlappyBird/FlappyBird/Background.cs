...
namespace FlappyBird
{
	...
	{	
		...
		//Public functions.
		public Background (Scene scene, String sceneName)
		{
			
			if(sceneName.Equals("startScene"))
			{
				sprites	= new SpriteUV[1];
				textureInfo = new TextureInfo("/Application/textures/start.png");
			...
			{
				sprites	= new SpriteUV[1];
				textureInfo = new TextureInfo("/Application/textures/dead.png");
				sprites[0] = new SpriteUV(textureInfo);
				sprites[0].Quad.S 	= textureInfo.TextureSizef;
			}
		...
		{			
			...
			if(sprites[0].Position.X < -width)
				sprites[0].Position = new Vector2(sprites[2].Position.X+width-1f, 0.0f);
			else
				sprites[0].Position = new Vector2(sprites[0].Position.X-1, 0.0f);	
			
			//Middle
			if(sprites[1].Position.X < -width)
				sprites[1].Position = new Vector2(sprites[0].Position.X+width-1f, 0.0f);
			else
				sprites[1].Position = new Vector2(sprites[1].Position.X-1, 0.0f);	
			
			//Right
			if(sprites[2].Position.X < -width)
				sprites[2].Position = new Vector2(sprites[1].Position.X+width-1f, 0.0f);
			else
